...
class DECA(nn.Module):
    ...
    def _setup_renderer(self, model_cfg):
        if getattr(self, 'renderer_ready', False):
            return
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        if rendering and not getattr(self, 'renderer_ready', False):
        ...
        if rendering:
            # Bug fix: pass codedict['light'] when render_orig is True so shading is applied
            if return_vis and render_orig and original_image is not None and tform is not None:
